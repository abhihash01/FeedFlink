...
def home():
    if 'logged_in' not in session or not session['logged_in']:
        ...
        return redirect(url_for('suggested_feed'))
...
def signin():
    ...
    if request.method == 'POST':
        ...
        if user == 1:
            flash('Please ensure all inputs are completed.', 'error')
        elif user == 2:
            flash('The email address is not valid.', 'error')
        elif user is None:
            flash('The email or password you entered is incorrect.', 'error')
        elif user:
            session['user_id']=user.id
            session['logged_in'] = True
            flash(f'Good to see you again, {user.firstname}!', 'success')
            return redirect(url_for('suggested_feed'))
        else:
            flash('An unexpected error has occurred. Please try again later.','error')
...
def news_preferences():
    ...
    if request.method == 'POST':
        if 'remove_category' in request.form:
            ...
            flash('Type has been removed!', 'success')
            ...
            flash('Type has been successfully added!', 'success')
    ...
    if 1 in user_sentiments:
        positive = True
    if -1 in user_sentiments:
        negative = True
    if 0 in user_sentiments:
        neutral = True
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if reg_code == 1:
            ...
            if otp == 1:
                flash('The email could not be sent. Please check your email address.', 'error')
            else:
                session['otp'] = otp
                ...
                flash(f'Thank you, {firstname}! A 6-digit confirmation code has been sent to your email address ({email}).', 'success')
...
@app.route('/suggested-feed', methods=["GET", "POST"])
def suggested_feed():
    if 'logged_in' not in session or not session['logged_in']:
        return redirect(url_for('signin'))
    controller = WelcomeController()
    ...
    return render_template('suggested_feed.html', 
